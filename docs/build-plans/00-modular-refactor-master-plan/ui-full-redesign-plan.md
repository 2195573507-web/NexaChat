...
This is a historical UI redesign plan. It remains useful for UI audit method, no fake capability labels, compact desktop-tool styling, and route-leak/overflow checks.
...
- PDF, Office, OCR, external vector databases, arbitrary MCP execution, and Agent sandbox are not completed capabilities.
...
| 路由兼容 | 历史计划当时记录 `/ -> /workspace/overview` alias；当前事实是 `/ -> /chat/conversations` | `src/shared/navigation.ts` | 旧 alias 清理方向仍可参考。 | 新 UI 不得新增旧链路备用实现；当前应保持 chat-first root。 |
