...
本路线图描述未来如何安全补齐能力，不表示这些未来能力已经在当前版本完成。本轮已完成其中一部分 P1/P2 项，下面总表只把已经真实接入、测试和文档同步的能力标为“已完成基础版”。
...
|---|---|---|---|---|
| Gateway `/v1/responses` | 已完成 basic text 版本；不支持 tools、多模态、background、advanced reasoning | 完整 OpenAI Responses API 兼容仍需后续扩展 | 中 | P1 已完成基础版 |
| Chat true streaming UI | 已有 typed stream event、局部 chunk 更新、取消与 late chunk 防护 | 仍需继续优化高频 token 渲染和更多 provider 真实流式 QA | 中 | P1 已具备基础链路 |
| IPC progress / partial update | Chat stream 与 task progress DTO 已存在并有 contract tests | 通用长任务跨 Knowledge/Data/Agent 的复用还需后续收敛 | 中 | P1 已具备基础链路 |
| Anthropic native provider | 已完成 native first version：文本 chat、基础 streaming、model list/fallback、key 验证和错误映射 | tool use、vision、高级 reasoning 未实现 | 中 | P1 已完成基础版 |
| Gemini native provider | 已完成 native first version：generateContent、基础 streaming、模型列表/fallback、key 验证和错误映射 | 多模态、函数调用和高级安全策略 UI 未实现 | 中 | P1 已完成基础版 |
| Other non-OpenAI-compatible providers | 已形成 registry 扩展点，当前接入 OpenAI-compatible、Anthropic、Gemini | 更多协议、embedding/tool/vision 能力声明仍需逐项适配 | 中高 | P2 部分完成 |
...
- 确认实现和文档中没有硬编码本机仓库绝对路径；路径必须从运行时、配置或 Git 根目录解析。
...
`/v1/responses` basic text 版本已在本轮实现。它不再是预留接口；当前只支持基础文本 input/output，并通过现有 Chat Provider 链路执行、记录 request log、usage 和 Gateway audit。它仍不能描述为完整 OpenAI Responses API 兼容。

#### Target behavior

- 已支持基本 text input。
- 已将基础 text input 映射到内部 chat completion 能力。
- 已返回清晰、稳定、可审计的响应结构。
- 继续明确标注为 basic version，不宣称完整 OpenAI Responses API 兼容。

#### Implementation boundary

- 当前第一版只覆盖基本文本输入和普通文本输出。
...
- Gateway tests 覆盖 auth/scope、basic success、unsupported fields、invalid input、audit/logging。
...
Provider adapter registry 已在本轮建立，统一接入 OpenAI-compatible、Anthropic native 和 Gemini native 的 chat、基础 streaming、模型列表、健康测试、请求摘要和错误映射。后续新增 provider 应继续通过 registry 扩展，避免在业务层散落 provider-specific 判断。
...
第一版 Anthropic native adapter 已覆盖文本 chat、基础 streaming、model list 或文档化的 model list fallback、API key 验证和错误映射。不要宣称完整 tool use、vision 或高级 reasoning 能力。

### Gemini native adapter first version

第一版 Gemini native adapter 已覆盖基础文本 generateContent、基础 streaming、模型列表或 fallback、API key 验证和错误映射，并通过 capability flags 保持边界清晰。不要把多模态或函数调用写成已完成，除非后续真实实现并验证。
...
- Anthropic native 和 Gemini native 已完成 text chat、基础 streaming 或明确 fallback、health check、错误映射。
- UI 显示 provider capability，不夸大未实现能力。
...
|---|---|---|---|---|
| 1 | Chat true streaming + IPC partial update + `/v1/responses` basic version | 已完成 Chat 局部流式基础链路、统一 stream/progress DTO、Gateway basic responses | 直接改善核心 Chat-first 体验，并为后续长任务事件打基础 | 中 |
| 2 | Provider adapter registry + Anthropic/Gemini native first version | 已完成多协议 provider registry、Anthropic/Gemini 文本与基础 streaming、错误映射 | 让多模型能力从 OpenAI-compatible 扩展到真实原生协议 | 中 |
...
- 不为了文档好看而夸大 runtime code 的真实能力。
- 不把尚未实现的功能写成已实现。
- 不无理由修改 package dependencies。
- 不删除 existing docs。
- 不宣称未完成功能已经完成。
- 不创建重复 roadmap 文件。
- 不隐藏权限问题。
