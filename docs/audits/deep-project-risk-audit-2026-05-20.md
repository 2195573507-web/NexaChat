...
- 实际仓库根目录：Git 检测到本机根目录为 `D:/NexaChat`；本报告的证据路径统一使用仓库相对路径，避免在正文中扩散本机绝对路径。
- 审计开始时间：2026-05-20 16:24:37 +08:00。
- 审计结束时间：2026-05-20 17:24:07 +08:00 写稿前安全检查；报告写入和最终验证另行记录在结尾。
- 实际耗时：约 60 分钟主动审计与交叉验证。
- 3 小时目标完成情况：未完整达到 3 小时。原因是当前 Codex 会话和上下文预算在约 1 小时深度审计后已进入压缩交接，用户随后将 Agent 数量限制为最多两个；本报告如实记录实际耗时，并基于两名专职审计 Agent 与主审静态复核给出最深可完成结论。
- 分支：`main`。
- HEAD：`9ff96dfcd1ea62d32c53d553d768e64f1b9fb392`。
- origin/main：`9ff96dfcd1ea62d32c53d553d768e64f1b9fb392`，写稿前与本地 HEAD 一致。
...
- 是否追加 `PROJECT_PROGRESS.md`：是，仅追加一条审计 trace。
- 是否运行 typecheck/test：已运行 `npm.cmd run typecheck` 与 `npm.cmd run test`，均通过；未运行 build/package/release。
- 是否发现其他任务并行运行：写稿前未发现 `.git/index.lock`、冲突标记或当前工作树变更；两个审计 Agent 曾在不同时间观察到不同状态，其中 Agent A 的 dirty 备注属于较早只读观察，最终写入前以重新检查到的干净状态为准。

## 2. 执行摘要

- 综合健康度评分：7.4 / 10。
- 最大风险结论：项目已经具备较清晰的 chat-first 产品事实、IPC 权限映射、secret 存储防线、Gateway/Knowledge/Tools 能力边界文案与测试基础；但核心实现仍受 `ServiceContext` 宽上下文、Dashboard/Workspace 兼容模型、Gateway 复用 Chat 写路径、Electron `sandbox: false`、Data restore 语义不够完整、Tools/MCP fixture-only 与发布链路未形成真实 installer/signing/update 能力的影响。
...
用户将 Agent 数量限制为最多两个，因此本次使用两个专职 Agent，并由主审串联 10 个原始审计角色的覆盖面。

| Agent | 职责覆盖 | 读取的主要文件/目录 | 交叉验证方式 |
| --- | --- | --- | --- |
| Agent A：架构/安全/数据/Provider | Electron main/preload/IPC、ServiceContext、serviceRegistry、Gateway/provider、secret、data restore、DB schema | `src/main/index.ts`、`src/preload/index.ts`、`src/shared/api.ts`、`src/main/ipc.ts`、`src/shared/ipc.ts`、`src/main/services/*`、`src/main/database/*`、`src/main/adapters/*` | 用 schema、service、tests、README/current architecture 互相核对；区分文档事实与代码兼容层 |
| Agent B：产品/UI/RAG/Tools/发布 | Chat/Models/Knowledge/Tools/Gateway/Data/Settings 的产品边界、UI/UX、可访问性、文档、测试、发布脚本 | `src/renderer/modules/*`、`src/renderer/components/*`、`src/shared/i18n.ts`、`tests/*`、`scripts/*`、`docs/*`、`package.json` | 用 UI 文案、测试断言、runtime constants、发布脚本和历史审计报告交叉验证 |

主审补充的 10 个角色覆盖：架构边界、产品模块行为、安全隐私、Gateway/provider、Chat streaming、Knowledge/RAG、Tools/MCP、UI/UX、性能测试、发布文档维护。

## 5. Top 30 风险清单

### R-01：`ServiceContext` 成为事实 god object

- 所属模块：架构 / 主进程服务层。
- 严重级别：P1。
- 置信度：高。
- 证据文件路径：`src/main/services/serviceContext.ts`、`src/main/services/serviceRegistry.ts`、`tests/store-boundaries.test.ts`。
- 风险说明：`ServiceContext` 既持有 DB/repositories，又承担 retrieval、embeddings、data/backup、secret/audit、fixture execution 等 helper，服务间能力边界偏软。
- 根因判断：早期为了快速聚合桌面工作台能力，采用共享上下文加 mixin service facade；后续模块扩展没有同步拆薄基础上下文。
- 影响范围：新功能容易跨模块调用内部 helper，增加回归面、测试复杂度和安全误用概率。
- 是否影响 `.exe` 打包：间接影响。打包不一定失败，但主进程初始化和迁移路径变更风险更高。
- 是否涉及安全/数据：涉及。secret、audit、data、gateway 能力位于同一宽上下文。
- 建议修复方案：分阶段把 Knowledge/Data/Gateway/Execution helpers 下沉到各自 domain service；`ServiceContext` 只保留 DB、transaction、repository access、permission/audit primitives。
- 推荐验证方式：import boundary test、service public method allowlist、store-boundaries regression、关键服务单元测试。
...
### R-04：Electron `sandbox: false`

- 所属模块：Electron / 安全边界。
- 严重级别：P2。
- 置信度：高。
- 证据文件路径：`src/main/index.ts`、`docs/architecture/current-architecture.md`、`README.md`。
- 风险说明：主窗口启用 `contextIsolation: true` 与 `nodeIntegration: false`，但仍保留 `sandbox: false`，安全基线低于更严格 Electron 配置。
- 根因判断：preload 桥接、Electron smoke、打包路径尚未完成 sandbox 兼容验证。
- 影响范围：renderer 被注入时，补偿控制仍依赖 preload allowlist 和主进程权限。
- 是否影响 `.exe` 打包：影响正式安全评审，不一定影响构建产物生成。
- 是否涉及安全/数据：涉及 renderer compromise 后的攻击面。
- 建议修复方案：开 sandbox 实验分支，逐项验证 preload API、file protocol、smoke、packaged app；不能只改配置。
- 推荐验证方式：sandbox-on Electron smoke、renderer API boundary tests、packaged smoke。
...
### R-09：Gateway 外部请求复用 Chat 写路径并持久化为会话/消息

- 所属模块：Gateway / Chat / Data。
- 严重级别：P2。
- 置信度：高。
- 证据文件路径：`src/main/services/localGateway.ts`、`src/main/services/chatService.ts`、`tests/gateway-provider-chain.test.ts`。
- 风险说明：Gateway `/v1/chat/completions` 与 `/v1/responses` 调用 `store.sendMessage()`，会生成 conversations/messages/request logs。
- 根因判断：复用 Chat provider chain 降低实现成本，但外部 API 语义和内部 UI conversation 语义耦合。
- 影响范围：外部工具调用可能污染 Chat 列表、增加本地数据保留和隐私解释成本。
- 是否影响 `.exe` 打包：不阻塞，但影响发布隐私说明。
- 是否涉及安全/数据：涉及用户内容留存。
- 建议修复方案：新增 Gateway execution layer，支持“持久化/非持久化”策略；UI 标注 Gateway-generated conversation。
- 推荐验证方式：Gateway side-effect tests、privacy export tests、conversation filtering tests。
...
### R-11：Anthropic/Gemini native adapter 能力有限

- 所属模块：Models / Provider。
- 严重级别：P2。
- 置信度：高。
- 证据文件路径：`src/main/adapters/providerAdapterRegistry.ts`、`README.md`、`docs/architecture/current-architecture.md`。
- 风险说明：OpenAI-compatible adapter 支持 embeddings/responses flags；Anthropic/Gemini native 当前覆盖文本 chat、streaming、models/fallback、key 验证和错误映射，不支持 embeddings/tools/vision/responses。
- 根因判断：第一版 native provider 以文本聊天为核心。
- 影响范围：模型选择 UI 和 Gateway/RAG 能力组合容易被用户误解。
- 是否影响 `.exe` 打包：不阻塞。
- 是否涉及安全/数据：低。
- 建议修复方案：Provider capability matrix 在 UI 和 docs 中保持强约束；扩展能力必须先补 runtime flags 和 tests。
- 推荐验证方式：provider-adapter tests、provider discovery tests、UI capability display tests。
...
### R-13：Chat 真流式依赖模型能力，fallback reveal 是 renderer 侧渐显

- 所属模块：Chat / Provider / UX。
- 严重级别：P2。
- 置信度：高。
- 证据文件路径：`src/main/services/chatService.ts`、`src/renderer/modules/ChatPage.tsx`、`src/renderer/modules/progressiveReveal.ts`、`tests/progressive-reveal.test.ts`。
- 风险说明：当 provider/model 不支持 streaming 或路径回退时，前端用 progressive reveal 模拟流畅感，不等于真实 GPT-style token streaming。
- 根因判断：需要兼容非 streaming provider 与现有 sendMessage response。
- 影响范围：体验预期、取消一致性、延迟解释。
- 是否影响 `.exe` 打包：不直接影响。
- 是否涉及安全/数据：否。
- 建议修复方案：UI 区分 “streaming” 与 “revealing completed response”；Provider capability 清晰展示。
- 推荐验证方式：streaming/non-streaming model UI tests、cancel path tests。
...
- 严重级别：P3。
- 置信度：中。
- 证据文件路径：`src/main/services/dashboardService.ts`、`src/main/repositories/*`、`src/renderer/modules/*`、`tests/pagination-aggregation-repositories.test.ts`。
- 风险说明：部分列表使用 repository pagination 或 fixed slice，但聚合 snapshot 仍包含 recent slices；不同模块“最近 N 条”的行为不一致。
- 根因判断：渐进优化未统一分页 contract。
- 影响范围：长时间使用后，用户找旧记录困难，性能上限不清。
- 是否影响 `.exe` 打包：不影响。
- 是否涉及安全/数据：低。
- 建议修复方案：制定统一 list query contract、total/hasMore/cursor；UI 显示最近记录与完整历史入口。
- 推荐验证方式：pagination contract tests。
- 建议 owner/agent：Performance / UX Agent。

### R-28：测试质量较好，但缺少 live provider/cloud/packaged release 矩阵

- 所属模块：Testing / Release readiness。
- 严重级别：P2。
- 置信度：高。
- 证据文件路径：`tests/*`、`package.json`、`docs/testing/validation-checklist.md`。
- 风险说明：本地 unit/integration/UI tests 覆盖强，但没有真实 provider 凭据矩阵、签名 installer、自动更新、跨机器 packaged smoke 的完整 CI 证据。
- 根因判断：local-first 项目安全地避免真实 secret，但 release 前需要受控环境验证。
- 影响范围：正式发布时 provider 兼容、系统环境、安装器行为不确定。
- 是否影响 `.exe` 打包：影响正式 release readiness。
- 是否涉及安全/数据：涉及 secret handling 和用户数据路径验证。
- 建议修复方案：建立可选 secrets CI matrix、packaged smoke、installer content scan、provider contract mocks + live opt-in。
- 推荐验证方式：test:package-smoke、test:installer-smoke、verify:release、manual signed installer smoke。
- 建议 owner/agent：QA / Release Agent。

### R-29：当前 release 不是签名安装器，也没有自动更新

- 所属模块：Release / Packaging。
- 严重级别：P2。
- 置信度：高。
- 证据文件路径：`package.json`、`scripts/package-win-unpacked.mjs`、`scripts/create-installer-script.mjs`、`docs/packaging/windows-installer-packaging-plan.md`、`.gitignore`。
- 风险说明：已有 `package:win-unpacked` 和 PowerShell installer script 方向，但无 NSIS/electron-builder 配置、无 code signing、无 auto-update。
- 根因判断：打包路线仍在计划和 smoke 阶段。
- 影响范围：用户安装体验、SmartScreen、升级、卸载、发布可信度。
- 是否影响 `.exe` 打包：直接影响正式 `.exe` 发布。
- 是否涉及安全/数据：涉及 artifact 内容排除和签名信任。
- 建议修复方案：引入正式 installer config；定义 file exclusions、signing policy、update policy；先做 unsigned internal build，再做 signed release。
- 推荐验证方式：package smoke、installer smoke、artifact content scan、signature verification。
...
- `ServiceContext` god-object：不阻塞生成内部 `.exe` smoke，但阻塞“可长期维护的正式 release”判断。它放大了安全、数据、Gateway、Knowledge 改动的交叉回归风险。
...
主要风险在于实现边界仍是宽上下文：`ServiceContext` 内聚合 DB、repositories、permission、audit、retrieval、embedding、data mobility、secret、fixture execution 等横跨多个模块的 helper。`DashboardService(ServiceContext)` 仍是 mixin 链第一层，`getSnapshot()` 聚合几乎所有模块状态。该设计在单机桌面应用初期可接受，但随着 Gateway、RAG、MCP、Data restore 继续扩展，未来维护成本会上升。

建议采用“保留 facade，拆薄上下文”的路线，而不是一次性大重写。每次重构只移动一个 domain helper，并用 boundary tests 锁住行为。
...
- `AppApi` 面较宽，renderer compromise 后可尝试大量 privileged actions。
- 默认本地 session/RBAC 不能被理解为真实多用户认证。

建议下一步先做 sandbox compatibility branch，不要直接在 main 改配置。
...
- 取消路径已处理 `activeChatControllers`，但 Gateway streaming、UI cancel、provider abort 的一致性仍应继续用 tests 锁住。
...
- UI 在无 embedding provider 时应预先说明 fallback，而不是先显示 vector。
...
2. Unsigned installer trial：先补 installer config、artifact content scan、userData isolation、secret fallback gate。
3. 正式发布：必须有 code signing、SmartScreen 预期说明、auto-update policy、clean checkout CI、packaged smoke、rollback/install/uninstall 测试。
...
- 未完整执行 3 小时；实际深度审计约 60 分钟。限制来自当前会话上下文压缩、用户将 Agent 数量改为最多两个、以及禁止生成/覆盖构建产物的边界。
...
但它也还没有达到“可以直接正式发布签名 `.exe`”的状态。最需要警惕的不是单个小 bug，而是几个长期风险叠加：宽 `ServiceContext`、Dashboard/Workspace 兼容模型未完全降级、Gateway 外部 API 和 Chat 内部持久化耦合、Electron sandbox 未启用、Data full restore 未实现、Tools/MCP 仍是 fixture-only、release installer/signing/update 仍不完整。
