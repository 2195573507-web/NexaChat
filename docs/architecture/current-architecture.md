...
- Gateway: 本地 OpenAI-compatible endpoints，当前支持 `/v1/models`、`/v1/chat/completions`、`/v1/embeddings` 和 `/v1/responses` basic text。`/v1/responses` 会映射到现有 Chat Provider 链路并记录 request log、usage 和 Gateway audit；不支持 tools、多模态、background 或 advanced reasoning。
