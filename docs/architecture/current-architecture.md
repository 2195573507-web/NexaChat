...
- SQLite 连接启用 `foreign_keys`、`busy_timeout=5000` 和 WAL，并写入 `schema_migration_runs` 迁移执行记录。`node:sqlite` 在当前 Node/Electron 组合下仍会输出 experimental warning，这是已知运行时风险，短期不切换数据库引擎。
...
- 补偿控制：renderer 只能访问 `window.nexachat` allowlist；preload 不暴露 raw `ipcRenderer`；主进程 IPC 使用集中 channel、arity/shape validation、权限检查和受控 handler。`nexachat://` 响应带 CSP 和 `nosniff`，主 session 默认拒绝 permission request。
...
- Models / Provider: OpenAI-compatible、Anthropic native 和 Gemini native 已通过统一 provider adapter registry 调用。Anthropic/Gemini 第一版覆盖文本 chat、基础 streaming、模型列表能力、API key 验证和错误映射；失败必须显式暴露为失败或 unknown，不再伪造模型列表成功 fallback；不代表 tool use、vision 或高级 reasoning 已完成。
...
- Tools / Agent / MCP: metadata registration、permissions、dry-run、内置 fixture-only execution、approval、trace/logging；不是任意 MCP/tool/code 执行。
- Data: import precheck、encrypted redacted backup package、restore preflight、有限 rollback records、diagnostics；有限 rollback 不是完整数据库恢复。
...
- 当前迁移 journal 能记录成功/失败，但尚未覆盖复杂锁竞争、跨版本失败恢复和替代 SQLite 运行时评估。
