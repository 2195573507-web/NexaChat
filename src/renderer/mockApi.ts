...
import { createMockModelApi } from './mockApiModels';
...
function retrieveKnowledge(state: MockState, input: KnowledgeRetrievalInput): KnowledgeRetrievalResult {
  ...
  const trace: KnowledgeRetrievalTrace = {
    ...
    knowledgeScopeJson: JSON.stringify({ mock: true }),
    candidateCount: candidates.length,
    vectorCandidateCount: 0,
    lexicalCandidateCount: candidates.length,
    finalCitationCount: scored.length,
    scoreSummaryJson: JSON.stringify({ rerank: 'disabled' }),
    timingsJson: JSON.stringify({ totalMs: 1, rerankMs: 0 }),
    errorCode: null,
    errorMessage: null,
    selectedChunkIdsJson: JSON.stringify(scored.map((chunk) => chunk.id)),
    resultCount: scored.length,
    fallbackReason: strategy === 'lexical' ? 'lexical_embedding' : 'mock_vector_unavailable',
    createdAt: timestamp,
  };
...
export function createMockApi(): AppApi {
  ...
  const modelApi = createMockModelApi({ state, now, createId, clone, findById, pushAudit, touchWorkspace });
  ...
  const api: AppApi = {
    ...
    disableModel: modelApi.disableModel,
    enableModel: modelApi.enableModel,
    deleteModel: modelApi.deleteModel,
    ...
    async sendMessage(input: SendMessageInput) {
      ...
      const usageRecord: UsageRecord = {
        ...
        inputTokens: userInputTokens,
        outputTokens: assistantOutputTokens,
        totalTokens: userInputTokens + assistantOutputTokens,
