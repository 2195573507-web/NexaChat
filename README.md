...
- Knowledge Base: text-like 导入、解析、chunk、provider-backed embedding/vector 检索（配置支持 embeddings 的 OpenAI-compatible 模型时）、lexical fallback 和 citations。
- Tools: MCP/Agent 注册、权限状态、dry-run、fixture-only execution、approval 和 trace/logging。
- Gateway: 本地 OpenAI-compatible Gateway，默认地址为 `127.0.0.1:8787`。
- Data: 配置导入预检、redacted export、encrypted redacted backup package、restore preflight、有限 rollback records 和 diagnostics。
...
- Knowledge Base 不是完整 PDF/Office/OCR/rerank RAG，也没有外部 vector DB；请先将资料转换为文本、Markdown、JSON 或 CSV。
- Knowledge Base 当前支持 text-like 导入、解析、chunk、本地 SQLite 向量记录、配置型 provider embedding 检索、lexical fallback 和 citations。
...
- `encrypted-redacted` backup 是加密的脱敏包，不包含 raw database、raw secrets 或完整数据库恢复能力。
...
npm.cmd run scan:release-safety
```

`package:win-unpacked`、`package:installer-script`、`package:release`、`test:package-smoke`、`test:installer-smoke`、`test:desktop-entry` 会触碰打包、安装器或发布产物路径，只能在明确允许生成 release artifacts 的发布工作流中运行。本轮 R-01 到 R-30 修复不生成 `.exe`、不生成安装器、不签名、不创建 auto-update feed；当前仓库也没有签名 Windows installer 或 auto-update feed。
...
- owner/operator/viewer 是本地桌面控制和审计模型，不是企业多用户身份认证系统。
